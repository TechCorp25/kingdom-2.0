...
If the input is a GitHub repository intake request, follow references/github-rest-intake.md.
...
Normalize each supplied or imported finding into a triage item.
...
Trace and test the complete claim against plausible supported paths.
...
the vulnerable condition is statically reachable under stated, supported preconditions
...
generate the valid triage-finding/v0 result internally
call open_codex_security_triage_results with that result
