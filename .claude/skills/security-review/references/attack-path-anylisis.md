...
Workflow
Load the per-scan threat model path from ../../references/scan-artifacts.md as the repo-specific threat-model source of truth. Start from this along with the potential findings. Both inputs are required for this workflow.
...
Drop any candidate whose final policy decision is ignore.
Save that finding's visible attack-path report to its per-finding attack-path analysis report path from ../../references/scan-artifacts.md.
