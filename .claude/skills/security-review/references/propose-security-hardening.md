...
refresh the target source and compare it with the recorded target revision or snapshot digest when one is available;
report material drift and update the proposal before coding;
turn the selected option into ordered work packages with explicit acceptance criteria, rollout, rollback, tests, and benchmarks;
preserve tactical protections needed during migration;
implement in reviewable phases and verify the mapped findings against the resulting code.
Quality Bar
