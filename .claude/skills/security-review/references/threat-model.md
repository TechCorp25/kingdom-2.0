...
Workflow
Resolve target_id, the current version (revision for an immutable Git tree, snapshot digest otherwise), and the repository-scoped threat model path using ../../references/scan-artifacts.md.
If the repository-scoped threat model exists, reuse it only when its final Repository and Version lines match those current values. Otherwise regenerate it.
Before inspecting repository source or generating a threat model, read ../../references/security-guidance.md and the policy resolved for the scan target. Resolve it first if the coordinator did not supply it.
If a threat model or authoritative security scan guidance is provided or referenced:
...
Otherwise, generate a repository-scoped threat model using the checklist below.
Before finalizing this phase, sanity-check that:
...
Append the exact Repository and Version lines from ../../references/scan-artifacts.md, then write the threat model to the repository-scoped path.
...
Included files
references/threat-model-guidance.md
text/markdown